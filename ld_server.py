...
pos_tabix = defaultdict(lambda: pysam.TabixFile(config['position_mapping'], parser=None))
...
def get_region_mapping(cpra, window):
    ...
    s = cpra.split(':')
    tabix_iter = pos_tabix[threading.get_ident()].fetch(s[0], max(1,int(s[1])-round(window/2)-1), int(s[1])+round(window/2), parser=None)
